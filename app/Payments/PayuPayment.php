...
class PayuPayment{
    ...
    public function makePayment($data){
        ...
        $order['notifyUrl'] = url('/order/payu/notify');
        $order['customerIp'] = $_SERVER['REMOTE_ADDR'];
        $order['merchantPosId'] = OpenPayU_Configuration::getMerchantPosId();
        $order['description'] = 'New order '.$data->id;
        $order['currencyCode'] = 'PLN';
        $order['totalAmount'] = $data->amount * 100;
    ...
    public function notify($request){
        ...
        if(isset($result)){
            ...
            if($order->getStatus() == 'SUCCESS'){
                $payu_order = $result->getResponse()->order;
                $order = Order::find($payu_order->extOrderId);
                if($order){
                    $payuPayment = PayuModel::where('local_order_id', $order->id)->first();
                    if($payuPayment){
                        $payuPayment->status = $payu_order->status;
